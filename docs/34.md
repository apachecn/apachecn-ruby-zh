...
让我们看看该程序的工作原理，在`s = Something.new`行中，我们创建了`Something`类的新实例变量`s`。 然后在下一行中执行`s.call_method "boo", 5`，在此行中，我们在`s`上调用一个名为`call_method`的方法，如果查看类`Something`，您会注意到在其中没有称为 call_method 的方法或函数 它，但程序不会抛出错误，异常或其他任何信息。
