...
如您从上面的示例中看到的代码，如果您输入关键字`private`，则在其下键入的内容将变为私有，对于受保护的内容也是如此。 我本可以编写如下所示的 public 方法
...
由于`hello`是带有数字的变量，因此将其与数字相乘会得到一个数字。 因此，您甚至可以看到乘法运算符根据情况采用多种形式或不同的函数。 就像这样，一个警察在家时对家人友善，当他受到轰击时，他的举止却有所不同。
